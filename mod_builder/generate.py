...
RGIGNORE_GENERATED_TXT_START = "# BEGIN generated by mod_builder/generate.py: generated .txt files"
RGIGNORE_GENERATED_TXT_END = "# END generated by mod_builder/generate.py: generated .txt files"
MODIFIER_DOC_PATH = (
    Path.home()
    / "Documents"
    / "Paradox Interactive"
...
def update_generated_txt_rgignore(generated_txt_paths, rgignore_path=RGIGNORE_PATH):
    """Maintain the generated .txt block in .rgignore without touching manual rules."""
    patterns = sorted(
        {
    ...
    block_lines = [
        RGIGNORE_GENERATED_TXT_START,
        "# Runtime .txt files generated from mod_builder/templates.",
        "# Search the source templates/config instead of these generated outputs.",
        *patterns,
        RGIGNORE_GENERATED_TXT_END,
    ]
    block_text = "\n".join(block_lines) + "\n"
    ...
    block_re = re.compile(
        rf"{re.escape(RGIGNORE_GENERATED_TXT_START)}.*?"
        rf"{re.escape(RGIGNORE_GENERATED_TXT_END)}\r?\n?",
        re.DOTALL,
    )
    if block_re.search(existing):
        updated = block_re.sub(block_text, existing)
    else:
...
def build():
        ...
        for filename in files:
            if filename.endswith('.txt.j2'):
                ...
                for variant in get_render_variants(tpl_path, output_name):
                    ...
                    generated_txt_paths.append(os.path.relpath(output_path, ROOT_DIR))
    ...
    update_generated_txt_rgignore(generated_txt_paths)
