...
def update_generated_output_rgignore(generated_output_paths, rgignore_path=RGIGNORE_PATH):
    ...
    ignored_suffixes = {".txt", ".yml"}
    patterns = sorted(
        {
            rgignore_pattern_for_output_path(path)
            for path in generated_output_paths
            if Path(path).suffix in ignored_suffixes
        }
    )
    block_lines = [
        RGIGNORE_GENERATED_OUTPUT_START,
        "# Runtime .txt and .yml files generated from mod_builder/templates.",
    ...
    rgignore_path.write_text(updated, encoding="utf-8")
